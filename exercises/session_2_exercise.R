...
#   
  x <- runif(100000000, 1, 100) 
  mean_x <- round(mean(x), 2) 
#   
#   1.  Try to understand what the code is doing by running it and
#   investigating the outputs.
#     - create 100 random (decimal) numbers between 1 and 100
#     - calculate the mean of the 100 random numbers and round it to 2 decimal places
#  
...
#     - 100 observations are not enough to obtain the same value for each run
#  
...
#     -> 10^8 observations are required to get a constant mean_x
#   
#   ## 2.2
#   
...
#   together?
#     - total amount of money: length(x+y) * 1,000 = 1,000 * 1,000 = 1,000,000
  length(z)*1000
#     
#     ## 2.3
#     
...
#   
  n <- 1e4
  scale <- 1.5e4
  income <- round( rbeta(n=n, shape1=4, shape2=10) * scale, 2)
#   
...
#     - shape1 shifts the distribution in horizontal direction
#     - shape2 stretches/compresses the distribution in vertical direction
#   
...
#   compute each person’s share of the total income of all people.
  income_total = sum(income) #42708319.76
  share_per_person = income_total/length(income) #4270.83
#   
...
#   <!-- -->
#     
  income_s <- sort(income) #sort individual income values in ascending order
  group <- c("Lower 10%", "Lower 50%", "Top 10%", "Top 1%")
  p <- c(.1, .5, .9, .99) #create vector with the four percentages

  boundary_not_rounded <- income_s[p*n]
  boundary <- round(income_s[p*n], 0) #get the income of the person that has the highest income of the first 10% group, 
                                      #the income of the person that gets the highest income in the group of the "poorest" half etc.
                                      #and round the values to integers (why round??)

  low10_m <- mean( income_s[c(1:(.1*n))] ) #get the average of the income of the "poorest" 10%
  low50_m <- mean( income_s[c(1:(.5*n))] )
  top10_m <- mean( income_s[c((.9*n):n)] )
  top1_m <- mean( income_s[c((.99*n):n)] ) #get the average of the income of the "richest" 1%

  means <-  round( c(low10_m, low50_m, top10_m, top1_m) , 0) #save the four averages in a vector and round to integers

  income_summary <- data.frame(group, boundary, means) #create dataframe with three columns (group, highest income and average income in each group)
  income_summary
#   
...
#   1.  Create a data frame with 5 variables and 5 rows.
  a <- seq(1,10,2)
  b <- seq(0,100,length.out=5)
  c <- seq(20,80,length.out=5)
  d <- seq(300,900,length.out=5)
  e <- seq(2,10,2)
  my_df <- data.frame(a,b,c,d,e)
  my_df
#   2.  Compute the arithmetic mean of each variable in the data set and
#   store all 5 means in one vector.
  df_means <- colMeans(my_df)
  df_means
...
#   the computed values and not also the respective variable names.
  means_df <- c(mean(my_df[,"a"]),mean(my_df[,"b"]),mean(my_df[,"c"]),mean(my_df[,"d"]),mean(my_df[,"e"]))
  means_df
#   
...
#   function `select`.
  ...
  ideal_cut_which <- diamonds[which(diamonds$cut =='Ideal'),]
  ideal_cut_filter <- filter(diamonds, cut == "Ideal")
